...
function tool_dxbot_show_help() {
    if [ "${showhelp}" == "yes" ]; then
        ...
        echo "*** USAGE ***"
        echo ""
        echo ""${0}" <path/to/crypto/blocknet/config/script.sh> <path/to/crypto/maker/config/script.sh> <path/to/crypto/taker/config/script.sh> <path/to/dxbot/config/script.sh> <path/to/dxbot/strategy/config/script.sh> <optional arguments>"
        echo ""
        ...
        echo " >> relative or absolute path for valid maker crypto currency configuration script"
        echo ""
        echo "<path/to/dxbot/config/script.sh>"
        echo " >> relative or absolute path for valid dxbot configuration script"
        echo ""
        echo "<path/to/dxbot/strategy/config/script.sh>"
        echo " >> relative or absolute path for valid dxbot strategy configuration script"
        echo ""
        echo "*** ARGUMENTS(optional) ***"
        echo ""
        echo "<update_source>"
        echo " >> existing source code of dxmakerbot will be updated with last version of repository"
        ...
        echo " >> existing target dxbot strategy generated files will be overwritten"
        echo ""
        echo "</path/to/custom/dxmakerbot/install/directory/>"
        echo " >> crypto currency maker bot scripts installation root path directory"
        echo " >> (default specified in config script)"
        echo " >> first path argument is always taken as custom dxmakerbot install directory"
        echo ""
        echo "<|noproxychains>"
        ...
        echo "example:"
        echo ""${0}" ./src/cfg.cc.blocknet.sh ./src/cfg.cc.blocknet.sh ./src/cfg.cc.litecoin.sh  ./src/cfg.dxbot.alfa.sh  ./src/cfg.strategy.block.ltc.sh mybasicstrategy BBBblocknetaddressBBB LLLlitecoinaddressLLL"
        echo ""
        exit 0
    fi
}
...
function tool_dxbot_process_arguments() {  #argc  #argv
    argc=${1}
    argv=${2}
    ...
    for (( j=5; j<argc; j++ )); do
        if [ "${argv[j]}" = "nofirejail" ]; then
            ...
            if [ "${cc_action_source}" = "" ]; then
                ...
                echo "INFO >> update source enabled"
            else
                echo "Invalid argument: $j ${argv[j]}"
                exit 1
            fi
            ...
            if [ "${cc_action_cc_config}" = "" ]; then
                ...
                echo "INFO >> update cc config enabled"
            else
                echo "Invalid argument: $j ${argv[j]}"
                exit 1
            fi
            ...
            if [ "${cc_action_xb_config}" = "" ]; then
                ...
                echo "INFO >> update xb config enabled"
            else
                echo "Invalid argument: $j ${argv[j]}"
                exit 1
            fi
            ...
            if [ "${cc_action_strategy}" = "" ]; then
                ...
                echo "INFO >> update strategy enabled"
            else
                echo "Invalid argument: $j ${argv[j]}"
                exit 1
            fi
        elif [[ "${argv[j]}" == *"/"* ]] ;then
            if [ "${cc_dxbot_install_dir_path}" = "" ]; then
                cc_dxbot_install_dir_path="${argv[j]}"
                echo "INFO >> cc install dir path >> ${cc_dxbot_install_dir_path}"
            else
                echo "Invalid argument: $j ${argv[j]}"
                exit 1
            fi
        else
            if [ "${cc_dxbot_naming_suffix}" = "" ]; then
                cc_dxbot_naming_suffix="${argv[j]}"
                echo "INFO >> using cc_dxbot_naming_suffix >> ${cc_dxbot_naming_suffix}"
            elif [ "${cc_address_maker}" = "" ]; then
                cc_address_maker="${argv[j]}"
                echo "INFO >> using cc_address_maker >> ${cc_address_maker}"
            elif [ "${cc_address_taker}" = "" ]; then
                cc_address_taker="${argv[j]}"
                echo "INFO >> using cc_address_taker >> ${cc_address_taker}"
            else
                echo "Invalid argument: $j ${argv[j]}"
                exit 1
            fi
...
function tool_dxbot_update_cc_cfg_xb_cfg() { # cfg.path # blocknet|maker|taker info #
...
function tool_dxbot_strategy_template_update() {  #strategy.file.path #opposite.bot.yes
    # check parameters
    ...
    for variable_name in ${strategy_conf_data}; do
        ...
        (test $? != 0) && echo "ERROR >> extract dxbot strategy config >> ${variable_name} >> failed"
        
        eval `echo "variable_value_opposite=\\$${variable_name}${opposite_bot_suffix}"`
        (test $? != 0) && echo "ERROR >> extract dxbot strategy opposite config >> ${variable_value_opposite} >> failed"
        
        eval `echo "variable_help_text=\\$${variable_name}${help_text_var_name_suffix}"`
        (test $? != 0) && echo "ERROR >> extract dxbot strategy help text >> ${variable_help_text} >> failed"
        
        eval `echo "variable_opposite_help_text=\\$${variable_name}${opposite_bot_suffix}${help_text_var_name_suffix}"`
        (test $? != 0) && echo "ERROR >> extract dxbot strategy opposite help text >> ${variable_opposite_help_text} >> failed"
        ...
        if [ "${variable_value}" != "" ]; then
            echo "FOR INFO >> processing dxbot strategy conf to update template variable >> ${variable_name} >> ${variable_value}"
            strategy_file=$(
            echo "$strategy_file" | sed \
                -e "s/{${variable_name}}/${variable_value}/g"
            )
            (test $? != 0) && echo "ERROR >> update dxbot template >> {${variable_name}} >> ${variable_value} >> failed" && exit 1
        fi
    done
    ...
    # write down updated dxbot startegy to file
    echo "${strategy_file}" > ${1}
    (test $? != 0) && echo "ERROR >> dxbot strategy file >> ${1} >> update failed" && exit 1
    
    echo "INFO >> dxbot strategy file >> ${1} >> update success"
